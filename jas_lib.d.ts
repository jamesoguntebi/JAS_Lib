...
declare module 'testing/fakes' {
  ...
  export class FakeProperties {
    ...
    setProperties(
        properties: Record<string, string>, deleteAllOthers?: boolean): void;
...
declare module 'testing/_simple_test' {
  ...
  export default abstract class SimpleTest {
    ...
    finish(): string[];
    /**
     * @param testFn A function that should throw if the test unit fails. It
     *     will be bound to `this`, allowing callers to conviently call
     *     `runUnit('description', this.test1)`.
     */
    private runUnit;
    /**
     * Throws an error that the Tester class always catches and rethrows, so
     * that when testing Tester, failures aren't suppressed.
     */
    protected fail(): void;
